...
//public class BinarySearchTree<AnyType extends Comparable<? super AnyType>>
{
    ...
    private BinaryNode<AnyType> insert( AnyType x, BinaryNode<AnyType> t )
    {
        ...
        if ( compareResult < 0 ) // if x is less than the node just checked 
        	t.left = insert( x, t.left ); 
        else if ( compareResult > 0 ) // if x is greater than the node just checked 
        	t.right = insert( x, t.right ); 
         //If a node with x already exists in tree, do nothing (duplicates ignored) 
    ...
    private boolean contains( AnyType x, BinaryNode<AnyType> t )
    {
    	if ( t == null )
            ...
            if ( compareResult == 0 ) // they are the same 
            	return true; 
            else if( compareResult < 0 ) // x is less than t
                return contains( x, t.left );
            else  // compareResult > 0  x is greater than t 
